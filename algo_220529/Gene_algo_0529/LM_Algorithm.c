...
double  LM_Algorithm(double Array0[9],double Tc,double Tc_uv_Slope_const_array[5],
...
//    Array[9]={0};
    ...
    double Delta_Lambda_B=8e-5,Delta_L_FWHM_B=8e-3
                    ,Delta_R_FWHM_B=1e-3,Delta_Lambda_S=1e-4
...
for (int  z= 0;z<LM_Iter;z++){
   ...
   for(int i= 0; i< num+1;i++){
    ...
    for(int i=0;i<num;i++){
          ...
          CRI_partial=CRI_calculate(Constrain_Flag,Tc,Tc_uv_Slope_const_array, Array_diff[0], Array_diff[1], Array_diff[2], Array_diff[3],Array_diff[4],Array_diff[5],Array_diff[6],Array_diff[7], TCS_width,Lower_limit, wavelength_interval, data_length, data_width, CIE1931, TCS,Daylight_SPD_1nm);
         *(Jacobian+i)=((100.0-CRI_partial)-(100.0-Array1[8]))/(Variance_Delta[i]*Array1[i]);
         Residuals_vector[i]=Array1[8]-CRI_partial;
...
//printf("HHHHHHHHHHHHHHHHHHH\n");
 ...
 //printf("inverse    SSSSSSSSSSSSSSSSSSSSSSSSSSS\n");
...
/*for(int i=0;i<num;i++){
   ...
   *(Delta+j) += (-1)**(Inverse+num*j+i) * *(Jacobian+i);
...
//if(Array[z%2][8]!=0.0  && Epson_temp<Epson &&Array[z%2][8]>Best[8]){
...
if(Array1[num]!=0.0 && Target_last>Target){
    ...
    if(Array1[num]>Best[num]){
      for(int i=0;i<num+1;i++)
...
else if(Array1[num]!=0.0 && Target_last<Target){

        Lambda_coef=Lambda_coef*2.0;
        ...
        Lambda_coe_temp=0.9*Lambda_coe_temp;
        Lambda_coef=0.001*Lambda_coe_temp;
