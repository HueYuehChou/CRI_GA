...
#include "LM_Algorithm.h"



double Genes_Algorithm(int Lower_limit,int data_length,int data_width,int wavelength_interval,int TCS_width,double CIE1931[data_length][data_width],double TCS[data_length][TCS_width],double Daylight_SPD_1nm[data_length][4],int Tc_lower,int Tc_interval,int Tc_point)
{
 srand(time(NULL));
...
for(double  therm=0;therm<Tc_point;therm+=1.0){
    ...
    int chrosome=100,Genes_number=8,Generation=300000;
...
double Cross_over(double Parent1,double Parent2,double iter,double Mutate){
    ...
    double    Mutation=Mutate*(2.0*rand()/RAND_MAX-1.0)*(1-(iter/300000.0));
